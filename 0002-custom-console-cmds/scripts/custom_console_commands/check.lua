...
function Check.saddle(prefab)
    prefab = prefab and prefab:lower() or "saddle_race"
    -- Allow users to simply use "basic", "race" and "war".
    if not prefab:find("^saddle_") then
        prefab = "saddle_"..prefab
    end
    -- If left fails (e.g. is `nil`), return that.
    return _G.Prefabs[prefab] and prefab
end
